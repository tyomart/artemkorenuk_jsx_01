...
const convert = (proTag) => {
    ...
    const convert0 = (cash,char) => {
        const proTag= cash.concat(char)
...
const processor2TXT = (inVal) => { // MOST HOT ISSUE - from where I have to take value to OUTPUT to  show processed only text 



    log('procTXT fired')
    let txtArray = Array.from(inVal)
    log(inVal, txtArray)
    ...
    //     if (spec(txtArray[i]) === true) { // if Spec

    //         return txtArray[i] //convert0(txtArray[i]) // convert
    //     } 
    //     else return  txtArray[i] // if normal, do nothing
        
    // } 
    

    return txtArray.length >0 
            ? txtArray.reduce((acc,elem,i,txt)=>{acc = () => {
                if (spec(elem) === true) {
...
const test2TXT = (str) => {
    //log( txtArray.isArray(), 'txtArray', txtArray,)
    return str.split('').reduce((acc,elem)=>{
        log('acc', acc, 'elem', elem,)
        return acc = acc + elem
        }
    ,'')
}
...
const handleTest = () => {
    ...
    let cca = '789'
    //const aTest = {0:'#',1:'#',2:'*', 3:'##'}
    const aTest = '0123456789</b>#asdf#gg'
   
   log('procTXT' , test2TXT(aTest))
...
useLayoutEffect(()=>{                 //triggering Text Processor and sync editor and ReadyTXT
    ...
    log('add Char', preIn)
        ...
        setReadyTXT(processor2TXT(preIn, readyTXT))
    }
...
//         // if (spec(txtArray[i+1]) === false) // if next char Spec

//         return '<h1>'//convert0(txtArray[i]) // convert

//         // else  {                         // if next is Spec

//         //     setCache(cache.concat(txtArray[i]))
